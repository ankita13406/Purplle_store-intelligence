...
The `--live` flag posts events to `http://localhost:8000/events/ingest` in real time. Without it, events are written to JSONL then replayed in batch.
...
To ingest a test event manually:
```bash
...
Navigate to **http://localhost:3000**. The dashboard polls all endpoints every 5 seconds and updates live.
...
Expected output: `PASSED` with coverage ≥ 70%. To run specific files:
```bash
...
The `data/events.jsonl` included in this repo is a synthetic dataset used for API validation without requiring the full pipeline to run. It was validated against real footage by running `detect.py` on the `STORE_BLR_002 CAM_ENTRY_01` clip (4,193 frames, ~2.3 min). After applying production-grade filters (confidence > 0.6, minimum 3 detections per visitor ID), the real clip yielded 8 confirmed unique individuals. The synthetic dataset estimates ~10 visitors — approximately **80% accuracy** against a single camera's ground truth. Cross-camera Re-ID deduplication across all 5 clips would bring the true count closer to the synthetic figure. See `docs/DESIGN.md` and `docs/CHOICES.md` (Decision 5) for full methodology.
...
See `docs/DESIGN.md` for full architecture rationale and `docs/CHOICES.md` for engineering decisions.
...
├── data/
│   └── events.jsonl    # Synthetic dataset (pipeline-derived, ~80% match to real footage)
├── docs/
...
- The synthetic `events.jsonl` is validated against real footage at ~80% accuracy — see the Synthetic Dataset Validation section above.
